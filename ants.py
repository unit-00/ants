...
# 1.1: The simulate method from AntColony class.
# 1.2: The bees leave the Hive when simulate calls on strategy method from the Hive class. When strategy is called,
#      it creates a list of exits when filtering through all of the places in AntColony class, and if the place's entrance is the Hive itself, then it becomes the exit for bees.
# 1.3: The places in the colony are laid out by first creating a place for the queen to reside in, this is the place where if the bees gets to, then the player loses. It then creates a layout with 
#      basic places based on the number of tunnels and the length of those tunnels, it creates a place and then registers those place. Lastly if it's at the end of a length, then it creates a bee entrance.
# 1.4: The insect's armor attribute is the amount of 'life' it has. When armor reaches 0, the insect gets removed from the current place it resides in.
...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            "*** YOUR CODE HERE ***"
            if isinstance(insect, QueenAnt) and insect.queen_tag == 0:
                return
            elif isinstance(insect, BodyguardAnt) and insect.ant is not None:
                self.ant = insect.ant
                return
            else:
                self.ant = None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        "*** YOUR CODE HERE ***"
        index = 0
        inspection = self.place
        while inspection is not hive:
            if inspection.bees and self.min_range <= index <= self.max_range:
                return random_or_none(inspection.bees)
            else:
                inspection = inspection.entrance
                index += 1
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self):
        ...
        "*** YOUR CODE HERE ***"
        self.queen_tag = QueenAnt.num_queens
    ...
    def action(self, colony):
        ...
        already_doubled = []
        def doubled(place):
            if place.ant is None or place.ant in already_doubled or isinstance(place.ant, QueenAnt):
                return
            else:
                if not place.ant.doubled:
                    already_doubled.append(place.ant)
        ...
        if self.queen_tag > 0:
            ...
            self.throw_at(self.nearest_bee(colony.hive))
            qp = self.place
            colony.queen, qp_entrance, qp_exit = QueenPlace(colony.queen, self.place), self.place.entrance, self.place.exit
            while qp_entrance is not None or qp_exit is not None:
                if qp_entrance is not None:
                    doubled(qp_entrance)
                    qp_entrance = qp_entrance.entrance
                if qp_exit is not None:
                    doubled(qp_exit)
                    qp_exit = qp_exit.exit
            doubled(qp)
